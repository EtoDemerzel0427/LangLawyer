 ...
 * 3. `auto&& x = expr;` (universal references) where `auto` deduces `T&` for lvalues and `T&&` for
 * rvalues.
 */
...
int main() {
    ...
    std::cout << std::is_const_v<decltype(c)> << '\n'; // 0, because the const is the int itself not
                                                       // the reference
    ...
    std::cout << std::is_const_v<std::remove_reference_t<
